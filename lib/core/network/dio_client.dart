...
import 'package:waslny_captain/core/network/auth_interceptor.dart';
import 'package:waslny_captain/core/utils/logger.dart';

/// Centralised Dio HTTP client for the Waslny Captain app.
///
...
class _LoggingInterceptor extends Interceptor {
  ...
  /// Masks sensitive header values (e.g. Authorization).
  Map<String, dynamic> _sanitiseHeaders(Map<String, dynamic> headers) {
    ...
    if (sanitised.containsKey('Authorization')) {
      final auth = sanitised['Authorization'] as String;
      sanitised['Authorization'] =
          auth.length > 20 ? '${auth.substring(0, 20)}…' : '***';
    }
    return sanitised;
  }
}
